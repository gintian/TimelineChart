/*!
 * TimelineChart v0.2.1 (http://softmetrixgroup.com/timeline)
...
var TimelineChart = (function() {
  ...
  var events = {
    startSwimlaneResize: function() {
      
    },
    stopSwimlaneResize: function() {
      
    },
    swimlaneResize: function() {
  ...
  function _createSwimlanes() {
    for(i = 0; i < _data.length; i++) {
      var height = _data[i].height;
      if(_data[i].autoHeight) {
        var calculatedHeight = (_data[i].single.length + _data[i].range.length) * 30 + 30;
        if(calculatedHeight > height) {
          height = calculatedHeight;
        }
      }
      var html = '<div class="row no-gutter swimlane-row" id="swimlane_'+i+'">';
      html += '<div class="col-xs-2 swimlane-title" style="background-color: '+_data[i].color+';height:'+ height +'px">';
      html += _data[i].title;
      html += '</div>';
      html += '<div class="col-xs-10 swimlane-content" style="height:'+ height +'px">';
      html += '</div>';
      html += '</div>';
      $(_containerSelector).append(html);
    }
  ...
  function _populateSwimlanes() {
    ...
    for(i = 0; i < _data.length; i++) {
      ...
      for(j = 0; j < _data[i].range.length; j++) {
        ...
        var order = "1." + id;
        if(_data[i].range[j].level) {
          order = _data[i].range[j].level + "." + j;
        } 
      ...
      for(j = 0; j < _data[i].single.length; j++) {
        ...
        var order = "1." + id;
        if(_data[i].single[j].level) {
          order = _data[i].single[j].level + "." + j;
        } 
        html.attr('data-order', order);
        html.attr('id', 'tlc_item_sng_' + id);
        swimlane.append(html);
      }
      var itemsForSort = swimlane.find('.swimlane-item');
      var tmp = [];
      itemsForSort.each(function(){
        tmp.push({
          'id': $(this).attr('id'),
          'sort': parseFloat($(this).data('order'))
        });
      }); 
  ...
  function _initSwimlanes() {
    $('.swimlane-row').resizable({
        ...
        resize: function( event, ui ) {
          events.swimlaneResize(event, ui);
        ...
        start: function( event, ui ) {
          events.startSwimlaneResize(event, ui);
        },
        stop: function( event, ui ) {
          events.stopSwimlaneResize(event, ui);
        }
    });
  }
