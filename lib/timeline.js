/*!
 * TimelineChart v0.1.0 (http://softmetrixgroup.com/timeline)
    ...
    function _populateSwimlanes() {
      ...
      for(i = 0; i < _data.length; i++) {
        ...
        for(j = 0; j < _data[i].range.length; j++) {
          ...
          var widthBeforeAfter =  0;
          if(cls.beforeWidth) {
            widthBeforeAfter += cls.beforeWidth;
          }    
          if(cls.afterWidth) {
            widthBeforeAfter += cls.afterWidth;
          }                
          var patternWidth = width - widthBeforeAfter;
          var beforeAfterDisplay = 'inline-block';
          if(width <= widthBeforeAfter) {
            patternWidth = width;
            beforeAfterDisplay = 'none';
          } 
          if(cls.beforeWidth) {
            html += '<div class="swimlane-item-range-before" \
                          style="display: '+beforeAfterDisplay+'; \
                                 height: '+cls.beforeHeight+'px; \
                                 width: '+cls.beforeWidth+'px; \
                                 background-image: url(\''+cls.before+'\'); \
                                 background-size: cover;"></div>';
          }
          html += '<div class="swimlane-item-range-pattern" \
                          style="display: inline-block; \
                               height: '+cls.patternHeight+'px; \
                               width: '+patternWidth+'px; \
                               background-image: url(\''+cls.pattern+'\')"></div>';
          if(cls.afterWidth) {                     
            html += '<div class="swimlane-item-range-after" \
                            style="display: '+beforeAfterDisplay+'; \
                                 height: '+cls.afterHeight+'px; \
                                 width: '+cls.afterWidth+'px; \
                                 background-image: url(\''+cls.after+'\'); \
                                 background-size: cover;"></div>';
          }
          html += '</div>';
          swimlane.append(html);
    ...
    function _updateSwimlaneItems() {
      ...
      $('.swimlane-item').each(function(){
        ...
        if($(this).hasClass('swimlane-item-single')) {
          ...
          if(width > widthBeforeAfter) {
            $(this).find('.swimlane-item-range-pattern').width(width-widthBeforeAfter);
            $(this).find('.swimlane-item-range-before').css('display', 'inline-block');
            $(this).find('.swimlane-item-range-after').css('display', 'inline-block');
          } else {
            $(this).find('.swimlane-item-range-pattern').width(width);
            $(this).find('.swimlane-item-range-before').hide();
            $(this).find('.swimlane-item-range-after').hide();
          }
        }
      });
    }
      ...
      Object.keys(_classes).forEach(function(key,index) {
        ...
        if(!_classes[key].parentClass) {
          if(_classes[key].class) {
            ...
            <i class='"+_classes[key].class+"' style='font-size: "+_classes[key].width+"'></i> \
            </div>";
          } else {
            if(_classes[key].image) {
                ...
                html += "<div class='timeline-legend-image'> \
                <img src='"+_classes[key].after+"' style='width: 15px; margin: 0 auto; display: inline-block;' /> \
                </div>";
            }
          }
    ...
    function _initContextMenu() {
      $.contextMenu({
            ...
            items: {
                ...
                ,"detach": {
                  name: "Detach label", icon: "fas fa-tag"
                }
                ,"rename": {
                  name: "Rename label", icon: "fas fa-edit"
                }
            }
        });
    }
